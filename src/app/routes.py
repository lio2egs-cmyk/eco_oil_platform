...
def generate_disposal_certificate_pdf(event, cert):
    try:
        ...
        c.drawString(1.5*cm, height - 2*cm, rtl('אמולסיה'))
        ...
        draw_rtl(c, width - 1.5*cm, height - 6.2*cm, event.client_name or '')
        draw_rtl(c, width - 1.5*cm, height - 6.9*cm, event.client_address or '')

        # הנדון
        draw_rtl(c, width - 1.5*cm, height - 8.2*cm, 'הנדון : אישור קליטת שפכי אמולסיה.', font='CalibriBold', size=12)

        # סעיף 1
        weight = event.weight_declared or ''
        packaging = f"{event.packaging_type or ''} {event.packaging_count or ''}"
        notes = event.notes or ''
        line1 = f"הרינו לאשר כי קלטנו במתקן הטיפול שלנו : {weight} טון שפכי אמולסיה; {packaging}. {notes}"
        draw_rtl(c, width - 1.5*cm, height - 9.5*cm, f'.1    {line1}')

        # סעיף 2
        draw_rtl(c, width - 1.5*cm, height - 10.3*cm, f'.2    החומר נפרק במפעלנו בתאריך : {event_date_str}')

        # סעיף 3
        carrier = event.carrier_name or ''
        draw_rtl(c, width - 1.5*cm, height - 11.1*cm, f'.3    החומר הועבר ע"י : {carrier} (להלן :"המוביל").')

        # פסקאות משפטיות
        y = height - 12.2*cm
        legal_lines = [
            'אישור זה אינו מהווה אישור של אקו אויל או אחריות של אקו אויל למקור השפכים או אופיים.',
            f'אישור זה אינו מהווה, בשום צורה שהיא, אישור של אקו אויל לכך שהשפכים אשר נפרקו במתקני אקו אויל מקורם : {event.client_name or ""}',
            f'האחריות על השפכים עד להגעתם למתקני אקו אויל הנה על {event.client_name or ""} בלבד ואקו אויל אינה אחראית בשום צורה שהיא על אופן שאיבת השפכים, אופן הטענתם ואופן ההובלה של השפכים.',
            f'על פי הצהרת : {carrier} (המוביל) החומר הגיע מחברת : {event.client_name or ""}.',
        ...
        # חתימה
        draw_rtl(c, width - 1.5*cm, y - 1*cm, 'בברכה')
        draw_rtl(c, width - 1.5*cm, y - 1.7*cm, 'חברת "אקו-אויל"')
